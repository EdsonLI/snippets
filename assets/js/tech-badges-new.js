...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function transformTechBadges() {
    ...
    techSpans.forEach(span => {
      ...
      // Caso especial para W3Schools - usar um SVG customizado com wrapper
      else if (tech === 'w3schools') {
        // Remover ícones existentes para W3Schools
        const existingIcons = span.querySelectorAll('iconify-icon, .w3schools-icon, .w3schools-icon-wrapper');
        existingIcons.forEach(icon => icon.remove());
        
        // Criar um elemento wrapper para o ícone do W3Schools
        const iconWrapper = document.createElement('div');
        iconWrapper.className = 'w3schools-icon-wrapper';
        
        // Adicionar o SVG diretamente no wrapper (sem span intermediário)
        iconWrapper.innerHTML = `<span class="w3schools-icon tech-icon-custom">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 54 50.8" width="16" height="16">
            <path d="M45.5,0H8.5A8.5,8.5,0,0,0,0,8.5v33A8.5,8.5,0,0,0,8.5,50h37A8.5,8.5,0,0,0,54,41.5V8.5A8.5,8.5,0,0,0,45.5,0" fill="#04AA6D"/>
            <path d="M27,16.31l9.2,16.4L27,33l-9.2-0.29Z" fill="#fff"/>
            <path d="M27,16.31,18.17,32.39,27,33l8.83-.59Z" fill="#fff"/>
            <path d="M9.47,5l13.7,23.8L38.16,5Z" fill="#fff"/>
          </svg>
        </span>`;
