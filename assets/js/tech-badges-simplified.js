/**
 * Tech Badges - Sistema simplificado para badges de tecnologia
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const TECH_ICONS = {
    ...
    'jquery': 'custom-jquery-svg', // Usando SVG customizado
    ...
    'w3schools': 'custom-w3schools-svg', // Usando SVG customizado
  ...
  // SVGs customizados
  const CUSTOM_SVGS = {
    'jquery': `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 128 128" width="16" height="16" style="aspect-ratio: 1/1;">
    ...
    'bootstrap': `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 128 128" width="16" height="16" style="aspect-ratio: 1/1;">
      <defs>
    ...
    'w3schools': `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 54 50.8" width="16" height="16" style="aspect-ratio: 1/1;">
      <defs>
    ...
    'css': `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 128 128" width="16" height="16" style="aspect-ratio: 1/1;">
  ...
  // Função para transformar spans em badges simplificadas
  function transformTechBadges() {
    ...
    techSpans.forEach(span => {
      ...
      // Casos especiais para SVGs customizados
      if (CUSTOM_SVGS[tech]) {
        // Remover ícones existentes
        const existingIcons = span.querySelectorAll(`iconify-icon, .${tech}-icon`);
        existingIcons.forEach(icon => icon.remove());
        
        // Inserir o SVG personalizado
        const tempDiv = document.createElement('div');
        tempDiv.innerHTML = CUSTOM_SVGS[tech].trim();
        const customIcon = tempDiv.firstChild;
        customIcon.classList.add(`${tech}-icon`, 'tech-icon-custom');
        
        // Inserir o ícone no DOM antes do texto
        span.insertBefore(customIcon, span.firstChild);
      }
      ...
      // SQL, PHP e Git são gerenciados pelos arquivos específicos
      else if (tech === 'sql') {
        ...
        if (!existingIcon) {
          // Adicionar o ícone no início do texto - versão simplificada
